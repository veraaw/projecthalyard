...
Emits analysis/integrity/findings.md (readable report, one section per check). The
same run also renders the embeddable HTML fragment consumed by
...
def markdown(results, context):
    ...
    L = [
        ...
        "",
        "Generated by `analysis/integrity/integrity_audit.py`. Read-only over `dataset/`: every entry below is a "
