# Larkhall Software  (C022)

- stage: Prospect | industry: Technology | owner: Imani Mkhize | deal value: $250,000 (CRM ARR potential) | by request: R1010 $250,000, R1062 $400,000
- CRM accounts: A1022 (larkhall.io)
- also goes by: nothing else
- 2 requests from 1 person wanting 2 different titles: Chief Technology Officer | Head of Developer Productivity

## 2. Where the files disagree

- R1062: filed "Intro sent" but intro_outcomes.csv has no row at all

## Currently routing to: nobody (no path to this company in the network)

- this cycle: R1010 unrouted (no path to this company in the network)
- nobody in the network reaches this company

## 3. Who can reach them

nobody in the network reaches this company

## 4. Chronology (9 events, 2 requests, newest first, as of 2026-09-06)

```
   2026-05-28  slack_threads.jsonl  Yusuf Petrossian     R1010 slack: "no idea sorry"
   2026-05-24  slack_threads.jsonl  Hana Nakashima       R1010 slack: "no idea sorry"
   2026-05-24  slack_threads.jsonl  Bertrand Vandermolen R1010 slack: "need help getting to Larkhall Software. Hugo Norrington-Isenberg is the Chief Technology Officer there, cold outbound is going nowhere"
   2026-05-24  intro_requests.csv   Bertrand Vandermolen R1010 raised by Bertrand Vandermolen (AE, EMEA): wants Chief Technology Officer, $250,000, Medium urgency, filed "Open"

   2026-05-13  slack_threads.jsonl  Yusuf Petrossian     R1062 slack: "adding Rafael Salcedo who might know"
   2026-05-09  slack_threads.jsonl  Imani Mkhize         R1062 slack: "no idea sorry"
   2026-05-09  slack_threads.jsonl  Bertrand Vandermolen R1062 slack: "who do we know at Larkhall Software? Head of Developer Productivity would be ideal but I'll take anyone senior"
!! 2026-05-09  intro_requests.csv   Bertrand Vandermolen R1062 raised by Bertrand Vandermolen (AE, EMEA): wants Head of Developer Productivity, $400,000, Critical urgency, filed "Intro sent"  [no intro in intro_outcomes.csv]

!! 2026-03-16  crm_accounts.csv     Imani Mkhize         last CRM touch on A1022  [174 days ago, nothing since]
```
