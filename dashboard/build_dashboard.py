...
<main>
<section id="trace">
  <p class="lede">Pick a company on the left, or search by name, alias, company id or CRM account id. Six sections follow: the header, where the files disagree (left out when they agree), who can reach them by strength, every event from <code>intro_requests.csv</code>, <code>slack_threads.jsonl</code>, <code>intro_outcomes.csv</code> and <code>crm_accounts.csv</code> oldest first, who needs to do what next, cheapest action first, and the additional investor and operator network around the company from <code>golden/network_orbit.csv</code> (left out when there is nobody; read-only, never scored or allocated). The same traces are written to <code>analysis/traces/</code> by <code>python3 build.py trace</code>.</p>
  {trace_fragment()}
</section>
