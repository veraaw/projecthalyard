...
const LP = (function () {
  ...
  function boot(D, root) {
    ...
    sec.route = `<section id="route"><h2>Route a Live Request <span class="foot">Paste a Slack message, or drop a <code>.jsonl</code> export, and see what the router would do with it</span></h2>
      <p class="lede">The browser applies the build's own rules, exported as data: the ${P.cues.length} cues from <code>golden/parse.py</code> score every company named, the highest positive score is the target, <code>golden/resolver.py</code>'s tables look the name up, offers in the replies are spotted with the build's pattern, and the company's paths from <code>supply_reach.csv</code> are ranked as the build ranks them. One row per thread: the company the build would resolve, any offer, who it would route to and what needs a human; click a row for the arithmetic and the ranked paths. A company nobody has filed yet but someone in the network reaches shows the paths the next rebuild would file for it. Nothing is written anywhere: export the preview and run the command to file it.</p>
      <div class="presets">Try a real shape:${P.route_presets.map((p, i) => `<button class="secondary" data-i="${i}">${esc(p.label)}</button>`).join('')}</div>
      <div class="ask"><textarea id="lp-route-text" rows="3" placeholder="Who do we know at … — or paste .jsonl lines, one {request_id, messages:[{ts,user,text}…]} per line" aria-label="Slack message or .jsonl"></textarea><button id="lp-route-go">Route it</button></div>
    ...
    sec.connectors = `<section id="connectors">${fold(`This Cycle, by Connector <span class="foot">Cycle ${esc(A.cycle)}: ${A.allocated} requests allocated in ${plural(A.batches.length, 'batch')}, one consolidated ask per connector, from <code>golden_allocation.csv</code> and <code>supply_reach.csv</code> · A tab per connector (${D.connectors.length - offRoster} on the roster${offRoster ? `, ${offRoster} off it` : ''}) with capacity, delivery rate, the drafted message and the batch behind it · The drafted messages are also on <a href="${esc(D.batch_page)}">Batched-Ask</a>${A.notify_count ? ` · ${plural(A.notify_count, 'request')} on a late-stage account someone other than its owner asked for: the owner gets a heads-up, nothing is held` : ''}</span>`)}`
      + tabs('connectors', [...D.connectors.map(c => ({ label: c.connector, n: c.capacity ? `${c.used}/${c.capacity}` : `${c.used}`, c })), { label: 'Aggregate', n: A.allocated, cls: 'agg' }], ({ c }, i) => c
        ...
        <div class="kpis"><div class="kpi"><div class="v">${c.capacity ? `${c.used} / ${c.capacity}` : c.used}</div><div class="l">${c.capacity ? 'capacity used this cycle' : 'asks this cycle'}</div><div class="s">${c.asked_this_cycle} asked + ${c.allocated_this_cycle} allocated${c.capacity ? ` · ${c.idle} idle` : ''}</div></div>
          ${rateKpi(c)}
          ...
          <div class="kpi"><div class="v">${c.allocated_this_cycle}</div><div class="l">in this cycle's ask</div><div class="s">${c.companies.length ? `${plural(c.companies.length, 'company')} · ${esc(c.batch_value_fmt)} · batch <code>${esc(c.batch_id)}</code>` : 'Nothing allocated'}</div></div></div>`
        ...
        + `<h3>The batch, by company</h3>` + (c.companies.length ? `<table><thead><tr><th>Company</th><th>Everyone wanted</th><th>Who is waiting</th><th>Path</th><th>Why this connector</th><th>Notify owner</th></tr></thead><tbody>`
          + c.companies.map(x => `<tr>${companyCell(x)}<td>${x.wanted.map(esc).join('<br>')}</td><td>${x.waiting.map(esc).join('<br>')}</td><td>${esc(x.path_type)}${x.contact ? `<br><span class="foot">${esc(x.contact)}</span>` : ''}</td><td class="foot">${esc(x.why)}</td><td>${notifyCell(x, i)}</td></tr>`).join('') + `</tbody></table>` : `<p class="empty">Nothing allocated to ${esc(c.connector)} this cycle</p>`)
        ...
        + A.all.map(x => `<tr>${companyCell(x)}<td><b>${esc(x.connector)}</b><br><a class="foot" href="${esc(D.batch_page)}#${esc(x.slug)}">The message</a></td><td>${x.wanted.map(esc).join('<br>')}</td><td>${x.waiting.map(esc).join('<br>')}</td><td>${esc(x.path_type)}${x.contact ? `<br><span class="foot">${esc(x.contact)}</span>` : ''}</td><td>${notifyCell(x, 'all')}</td></tr>`).join('')
        + `</tbody></table>`)
      + `</details></section>`;
    ...
    const EXCEPTION_NOTE = { 'no path to this company in the network': 'Not routable this cycle (sourcing issue vs. allocation): the named person is who to ask whether they know anyone',
    ...
    for (const e of A.exceptions) {
      ...
      const unresolved = e.reason.startsWith('company'), blocked = e.rows.some(r => r.blocked_reason);
      const note = r => r.detail || r.best_path || (unresolved ? r.company_as_written || '(nothing parseable)' : '');
      const withNote = e.rows.some(note);
      sec.exceptions += `<h3>${esc(EXCEPTION_TITLE[e.reason] || cap1(e.reason))} <span class="foot">${e.count} · ${esc(e.value_fmt)}</span></h3>${EXCEPTION_NOTE[e.reason] ? `<p class="foot">${esc(EXCEPTION_NOTE[e.reason])}</p>` : ''}<table><thead><tr><th>Request</th><th>Company</th>${unresolved ? '' : '<th>CRM stage</th>'}<th>Wanted</th><th>Who</th><th>Value</th><th>Urgency</th><th>Status</th>${blocked ? '<th>Blocked on</th>' : ''}<th>Who covers this sector</th>${withNote ? `<th>${unresolved ? 'As written' : 'Note'}</th>` : ''}</tr></thead><tbody>`
        + e.rows.map(r => `<tr><td class="rid">${esc(r.request_id)}</td><td>${co(r)}${routedHere(r)}</td>${unresolved ? '' : `<td>${esc(r.crm_stage)}</td>`}<td>${esc(r.target_title)}</td><td>${esc(r.requested_by)}</td><td>${esc(r.value_fmt)}</td><td>${esc(r.urgency)}</td><td>${esc(r.status)}</td>${blocked ? `<td>${esc(r.blocked_reason)}</td>` : ''}<td>${cover(r.sector_cover)}</td>${withNote ? `<td class="foot">${esc(note(r))}</td>` : ''}</tr>`).join('')
        + `</tbody></table>`;
    }
