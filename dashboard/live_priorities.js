...
const LP = (function () {
  ...
  function boot(D, root) {
    ...
    const companyCell = (c, tag = '') => `<td>${co(c)}${tag ? ` ${tag}` : ''}<br><span class="foot">${esc(c.value_fmt)} · ${esc(c.urgency)} · ${c.request_ids.map(esc).join(', ')}</span>${retryTag(c)}</td>`;
    ...
    sec.introduced = `<section id="introduced">${fold(`Already Introduced — Extend the Intro <span class="foot">${plural(I.requests, 'request')} on ${plural(I.count, 'company')}, ${esc(I.value_fmt)} · An intro already landed there (meeting booked, or sent within ${I.days} days), so no connector slot is spent</span>`)}
      ...
      + (I.retries.length ? `<h3>Back in the queue as a retry <span class="foot">${plural(I.retry_requests, 'request')} on ${plural(I.retries.length, 'company')} whose last intro fizzled · Nothing to tick here: a row greys out once its asks are ticked under <a href="#connectors">the connector</a> or <a href="#top">Top Priorities</a></span></h3>
