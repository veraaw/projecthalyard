...
const LP = (function () {
  ...
  function boot(D, root) {
    ...
    // what is going out. No box here: a company row is done-state only, lit once every request in it
    // has its ask_sent tick (Top Priorities, the connector's queue or page)
    const A = D.asks;
    ...
    sec.asks = `<section id="asks">${fold(`Current Asks <span class="foot">Cycle ${esc(A.cycle)}: ${A.allocated} requests allocated in ${plural(A.batches.length, 'batch')}, one consolidated ask per connector, from <code>golden_allocation.csv</code> and <code>supply_reach.csv</code> · the drafted messages are on <a href="${esc(D.batch_page)}">Batched-Ask</a>${A.notify_count ? ` · ${plural(A.notify_count, 'request')} on a late-stage account someone other than its owner asked for: the owner gets a heads-up, nothing is held` : ''}</span>`)}`
      + tabs('asks', [...A.batches.map(b => ({ label: b.connector, n: b.size, b })), { label: 'Aggregate', n: A.allocated, cls: 'agg' }], ({ b }, i) => b
        ...
        + b.companies.map(c => `<tr ${mirror(state, batchTicks(c, b.connector))}><td>${co(c)} ${sent}<br><span class="foot">${esc(c.value_fmt)} · ${esc(c.urgency)} · ${c.request_ids.map(esc).join(', ')}</span>${retryTag(c)}</td><td>${c.wanted.map(esc).join('<br>')}</td><td>${c.waiting.map(esc).join('<br>')}</td><td>${esc(c.path_type)}${c.contact ? `<br><span class="foot">${esc(c.contact)}</span>` : ''}</td><td class="foot">${esc(c.why)}</td><td>${notifyCell(c, i)}</td></tr>`).join('')
        + `</tbody></table>`
        ...
        + A.all.map(c => `<tr ${mirror(state, batchTicks(c, c.connector))}><td>${co(c)} ${sent}<br><span class="foot">${esc(c.value_fmt)} · ${esc(c.urgency)} · ${c.request_ids.map(esc).join(', ')}</span>${retryTag(c)}</td><td><b>${esc(c.connector)}</b><br><a class="foot" href="${esc(D.batch_page)}#${esc(c.slug)}">The message</a></td><td>${c.wanted.map(esc).join('<br>')}</td><td>${c.waiting.map(esc).join('<br>')}</td><td>${esc(c.path_type)}${c.contact ? `<br><span class="foot">${esc(c.contact)}</span>` : ''}</td><td>${notifyCell(c, 'all')}</td></tr>`).join('')
        + `</tbody></table>`);
    sec.asks += `</details></section>`;


    // who is carrying it
    sec.connectors = `<section id="connectors">${fold(`Roster Connector Capacity <span class="foot">One panel each: queue this cycle, capacity used against stated capacity, delivery rate, what they are already sitting on. The coloured tabs at the top open each connector's own page</span>`)}`
      + tabs('connectors', D.connectors.map(c => ({ label: c.connector, n: `${c.used}/${c.capacity}`, c })), ({ c }) => `
        ...
        + composeBlock(c.batch_ask, `ask-${c.slug}`)
        + `<h3>Queue this cycle <span class="foot">Tick a row once the ask has gone out: the same tick as <a href="#top">Top Priorities</a> and <a href="${esc(c.page)}">their page</a></span></h3>` + (c.queue.length ? `<table><thead><tr><th></th><th>Request</th><th>Company</th><th>Wanted</th><th>For</th><th>Path</th><th class="num">Score</th><th>Value</th><th>Urgency</th></tr></thead><tbody>`
          + c.queue.map(q => { const t = askTick(X, q); return `<tr class="${doneClass(state, t)}">${tick(state, t)}<td class="rid">${esc(q.request_id)}</td><td>${co(q)}${retryTag(q)}</td><td>${esc(q.target_title)}</td><td>${esc(q.requested_by)}</td><td>${esc(q.path_type)}${q.contact ? ` <span class="foot">via ${esc(q.contact)}</span>` : ''}</td><td class="num">${esc(q.route_score)}</td><td>${esc(q.value_fmt)}</td><td>${esc(q.urgency)}</td></tr>`; }).join('') + `</tbody></table>` : `<p class="empty">Nothing allocated to ${esc(c.connector)} this cycle</p>`)
        + `<h3>Already sitting on</h3>` + sittingTable(c, X, state))
      + `</details></section>`;
    ...
    const EXCEPTION_NOTE = { 'no path to this company in the network': 'Not routable this cycle (sourcing issue vs. allocation). If someone got asked anyway — a roster connector whose path is not on file, or someone off the roster who offered — pick who and the row is ticked as an ask sent to them: after the next rebuild it leaves here, sits under that name as an outstanding ask, and counts against a roster connector’s capacity even past their stated number (flagged on their panel). Nobody’s path is added to the network by it.',
    ...
    sec.exceptions = `<section id="exceptions">${fold(`Unrouted Exceptions <span class="foot">${plural(A.exception_count, 'request')} not allocated in cycle ${esc(A.cycle)}: ${A.exceptions.map(e => `${e.count} ${esc((EXCEPTION_TITLE[e.reason] || e.reason).replace(' to this company in the network', '').toLowerCase())}`).join(', ')} · From <code>golden_allocation.csv</code></span>`)}`
    ...
    for (const e of A.exceptions) {
      ...
      sec.exceptions += `<h3>${esc(EXCEPTION_TITLE[e.reason] || cap1(e.reason))} <span class="foot">${e.count} · ${esc(e.value_fmt)}</span></h3>${EXCEPTION_NOTE[e.reason] ? `<p class="foot">${esc(EXCEPTION_NOTE[e.reason])}</p>` : ''}<table><thead><tr>${noPath ? '<th>Asked anyway · by whom</th>' : ''}<th>Request</th><th>Company</th>${withStage ? '<th>CRM stage</th>' : ''}<th>Wanted</th><th>Who</th><th>Value</th><th>Urgency</th><th>Status</th>${blocked ? '<th>Blocked on</th>' : ''}${noPath ? '<th>Who covers this sector</th>' : `<th>${e.reason.startsWith('capacity') ? 'Best path (no slot)' : e.reason.startsWith('company') ? 'As written' : 'Note'}</th>`}</tr></thead><tbody>`
        + e.rows.map(r => `<tr${noPath ? ` class="${doneClass(state, askTick(X, r))}"` : ''}>${noPath ? pickTick(state, askTick(X, { ...r, connector: '' }), names(r)) : ''}<td class="rid">${esc(r.request_id)}</td><td>${co(r)}${routedHere(r)}</td>${withStage ? `<td>${esc(r.crm_stage)}</td>` : ''}<td>${esc(r.target_title)}</td><td>${esc(r.requested_by)}</td><td>${esc(r.value_fmt)}</td><td>${esc(r.urgency)}</td><td>${esc(r.status)}</td>${blocked ? `<td>${esc(r.blocked_reason)}</td>` : ''}${noPath ? `<td>${cover(r.sector_cover)}</td>` : `<td class="foot">${esc(r.detail || r.best_path || (e.reason.startsWith('company') ? r.company_as_written || '(nothing parseable)' : ''))}</td>`}</tr>`).join('')
        + `</tbody></table>`;
    }
    sec.exceptions += `</details></section>`;

    // still in the cycle band: suggestions, not decisions. The box is the same ask_sent tick as an unrouted
    // exception's, connector pre-filled: one box per company covers its live requests
    ...
    sec.unrouted = `<section id="unrouted">${fold(`Suggested Unrouted Company Connectors <span class="foot">${U.unrouted_companies} companies with live requests nobody is allocated to, matched to each connector's stated focus areas</span>`)}
      <p class="lede">In-focus asks convert at <b>${esc(F.in_focus_pct)}</b> versus ${esc(F.out_focus_pct)} outside, yet only ${F.in_focus_asks} of ${F.total_asks} asks landed in focus. These are the names that finding points at. Tick a row if you did ask that connector for the company: it records one <i>ask sent</i> per live request there, the same tick as under <a href="#exceptions">Unrouted Exceptions</a>, and counts against their capacity even past the stated number. A company with a path also moves on the connector's page; one without moves in sourcing, from its trace.</p>`;
    for (const c of U.per_connector) {
      ...
      sec.unrouted += c.companies.length ? `<table><thead><tr><th></th><th>Company</th><th>Industry</th><th>Wanted</th><th>Waiting</th><th>Value</th><th>Why unrouted</th><th>Their way in</th><th>Action lives</th></tr></thead><tbody>`
        + c.companies.map(x => { const g = batchTicks(x, c.connector); return `<tr class="${groupDone(state, g)}">${groupTick(state, g)}<td>${co(x)}<br><span class="foot">${x.request_ids.map(esc).join(', ')}</span></td><td>${esc(x.industry)}</td><td>${x.wanted.map(esc).join('<br>')}</td><td>${x.waiting.map(esc).join('<br>')}</td><td>${esc(x.value_fmt)}</td><td class="foot">${x.reasons.map(esc).join('; ')}</td><td class="${x.has_path ? '' : 'foot'}">${esc(x.path)}</td><td class="foot">${x.has_path ? `<a href="${esc(c.page)}">${esc(c.connector)}'s page</a>` : x.href ? `Sourcing · <a href="${esc(x.href)}">trace</a>` : 'Sourcing'}</td></tr>`; }).join('') + `</tbody></table>`
