...
const LP = (function () {
  ...
  function route(text, P) {
    ...
    if (C.network) out.note = `${C.company_name} is not on file: no CRM account, never requested. The network reaches it${C.path_count ? ` (${plural(C.path_count, 'path')})` : ''}; filing this request creates the company and the next rebuild files the paths below, ranked as shown; whether the top connector has a slot that cycle is a separate question (see priority). Create the CRM account (see CRM Updates).${out.note ? ' ' + out.note : ''}`;
    return out;
  }
  ...
  function renderPreview(pv, filename, P, el) {
    ...
    let out = `<div class="kpis"><div class="kpi"><div class="v">${pv.count}</div><div class="l">threads</div><div class="s">${pv.rows.filter(r => r.filed).length} already filed</div></div>
      <div class="kpi"><div class="v">${pv.rows.filter(r => r.company_id || r.network).length}</div><div class="l">resolved to a company</div><div class="s">${pv.rows.filter(r => r.network).length ? `${pv.rows.filter(r => r.network).length} not yet on file · ` : ''}${pv.rows.filter(r => !r.filed && !r.company_id && !r.network).length} not</div></div>
