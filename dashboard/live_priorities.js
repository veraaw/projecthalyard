...
const LP = (function () {
  ...
  function previewThreads(jsonlText, P) {
    ...
    const rows = threads.map(t => {
      ...
      if (filed) {
        ...
        const tg = ex.target;
        if (!tg) { row.resolved_by = 'no company named in the ask'; human.push('no company named, so the build files it with no company_id'); }
        else {
  ...
  const sittingTable = (c, X, state) => c.sitting_on.length ? `<table class="top"><thead><tr><th></th><th>action</th><th>request</th><th>company</th><th>wanted</th><th>for</th><th>asked</th><th class="num">days</th><th>responded</th><th>value</th></tr></thead><tbody>`
    + c.sitting_on.map(s => { const k = followTick(X, s); return `<tr class="${s.quiet ? 'quiet' : doneClass(state, k)}">${s.quiet ? `<td title="${esc(actionLabel[k.action])} ${esc(s.nudged_on)}; back in ${c.quiet_days - s.days_since_nudged}d"></td>` : tick(state, k)}<td><b>${esc(s.action)}</b>${s.quiet ? `<br><span class="foot">${esc(actionLabel[k.action])} ${s.days_since_nudged}d ago</span>` : ''}</td><td class="rid">${esc(s.request_id)}</td><td>${co(s)}</td><td>${esc(s.target_title)}</td><td>${esc(s.requested_by)}</td><td class="date">${esc(s.asked_date)}</td><td class="num">${s.days_since_asked}</td><td>${s.responded ? '<b>yes, so nudge rather than re-ask</b>' : 'no'}</td><td>${esc(s.value_fmt)}</td></tr>`; }).join('') + `</tbody></table><p class="foot">Tick a row once you have nudged or chased; it is off the list for ${c.quiet_days} days after the next rebuild.</p>` : `<p class="empty">nothing outstanding</p>`;
  ...
  function boot(D, root) {
    ...
    // free: someone already volunteered
    const O = D.offer_gaps;
    sec.offers = `<section id="offers">${fold(`Already offered, needs response <span class="foot">${plural(O.count, 'request')}, ${esc(O.value_fmt)}: someone volunteered in the Slack thread and no ask was ever logged</span>`)}
      <p class="lede">These cost nothing to action, since someone already said yes. Reply in the thread and log the ask.</p>
    ...
    // no slot spent: a nudge
    const B = D.bottlenecks;
    sec.bottlenecks = `<section id="bottlenecks">${fold(`Core bottlenecks <span class="foot">${plural(B.count, 'ask')} where the connector said yes and never sent the intro, ${esc(B.value_fmt)}, from <code>intro_outcomes.csv</code></span>`)}
    ...
    sec.unrouted = `<section id="unrouted">${fold(`Unrouted company asks, per connector <span class="foot">${U.unrouted_companies} companies with live requests nobody is allocated to, matched to each connector's stated focus areas</span>`)}
    ...
    const K = D.checkins;
    const ownedBy = r => r.owned_by ? `<a class="foot" href="#${esc(r.owned_by)}">→ ${esc(r.owned_by_title)}</a>` : `<b>${esc(r.owner || 'nobody')}</b> <span class="foot">in the CRM</span>`;
    sec.checkins = `<section id="checkins">${fold(`Overdue a check-in <span class="foot">${plural(K.count, 'company')} with live requests and no CRM touch in ${K.days} days; ${K.unique} owned by no other section</span>`)}
      <p class="lede"><b>${plural(K.count, 'company')} with live requests ${K.count === 1 ? 'has' : 'have'} had no CRM touch in ${K.days} days. ${K.both} ${K.both === 1 ? 'has' : 'have'} had no intro ask either.</b> Those are being neither sold to nor networked into. ${K.owned} of them already sit in another section that owns the next move, and the pointer says which. The ${plural(K.unique, 'company')} nothing else owns ${K.unique === 1 ? 'is' : 'are'} at the top, and a check-in is the only action left on them.</p>
      <table class="top"><thead><tr><th>company</th><th>owner</th><th>stage</th><th>CRM last_touch_date</th><th>last intro ask</th><th>failed</th><th>live</th><th>action owned by</th></tr></thead><tbody>`
      + K.rows.map(r => `<tr class="${r.owned_by ? 'owned' : ''}"><td>${co(r)}</td><td>${esc(r.owner || 'nobody')}</td><td>${esc(r.stage)}</td><td class="date">${esc(r.last_touch_date || 'never')}${r.touch_days != null ? ` <span class="foot">${r.touch_days}d</span>` : ''}</td><td class="date">${esc(r.last_ask_date || 'never')}${r.ask_days != null ? ` <span class="foot">${r.ask_days}d</span>` : ''}</td><td>${r.failed.map(f => `<b class="${r.failed.length === 2 ? 'warn' : ''}">${esc(f)}</b>`).join(' + ')}</td><td>${r.live_requests} · ${esc(r.live_value_fmt)}</td><td>${ownedBy(r)}</td></tr>`).join('')
      + `</tbody></table></details></section>`;
  ...
  function renderPreview(pv, filename, P, el) {
    ...
    out += `<table class="preview"><thead><tr><th>request</th><th>posted · by</th><th>resolved company</th><th>offer in replies</th><th>would route to</th><th>needs a human</th></tr></thead><tbody>`
      + pv.rows.map(r => `<tr class="${r.flags.length ? 'flag' : ''}"><td class="rid">${esc(r.request_id)}${r.filed ? '<br><span class="foot">filed</span>' : ''}</td><td class="date">${esc(r.posted)}<br><span class="foot">${esc(r.requested_by)}</span></td><td>${r.company_id ? `${co(r)} <span class="foot">${esc(r.company_id)}</span>` : `<i>${esc(r.company_name || 'none')}</i>`}<br><span class="foot">${esc(r.company_as_written ? `"${r.company_as_written}" · ` : '')}${esc(r.resolved_by)}</span></td><td>${r.offers.length ? r.offers.map(o => `<b>${esc(o.who)}</b> <span class="foot">${esc(o.date)}</span><br><q>${esc(o.text)}</q>`).join('<br>') : '<span class="foot">none</span>'}</td><td>${r.route_to ? `<b>${esc(r.route_to)}</b><br><span class="foot">${esc(r.path)}</span>` : `<span class="foot">${esc(r.path || 'none')}</span>`}</td><td class="foot">${r.flags.length ? r.flags.map(esc).join('<br>') : 'nothing'}</td></tr>`).join('')
      + `</tbody></table>`;
    el.innerHTML = out;
