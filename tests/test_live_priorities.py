...
class PayloadTest(unittest.TestCase):
    ...
    def test_current_asks_match_golden_allocation(self):
        ...
        # a company is placed or runs out of room as one ask, so nothing else on the account is ever routed
        self.assertTrue(all(r["routed_here"] == [] for r in capacity["rows"]))
        self.assertIn("R1004", {r["request_id"] for r in capacity["rows"]}, "Marchford Clinics")
        r1041 = next(a for a in allocated if a["request_id"] == "R1041")
        self.assertEqual((r1041["allocated_to"], r1041["rides_with"]), ("Marcus Aldridge", "R1024"),
                         "R1041 rides with R1024 instead of waiting on capacity")
